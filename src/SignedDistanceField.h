...
public:
  ...
  inline float_index convertPoint3toCell(const Eigen::Vector3d& point) const {
    const double minX = origin_.x();
    const double maxX = minX + (field_cols_ - 1.0) * cell_size_;
    const double minY = origin_.y();
    const double maxY = minY + (field_rows_ - 1.0) * cell_size_;
    const double minZ = origin_.z();
    const double maxZ = minZ + (field_z_    - 1.0) * cell_size_;

    // compute the fractional indices for logging
    const double idxX = (point.x() - minX) / cell_size_;
    const double idxY = (point.y() - minY) / cell_size_;
    const double idxZ = (point.z() - minZ) / cell_size_;

    // check X
    if (point.x() < minX || point.x() > maxX) {
      std::ostringstream oss;
      oss << "X out of range: x=" << point.x()
          << " (valid [" << minX << "," << maxX << "]), idxX=" << idxX;
      throw std::out_of_range(oss.str());
    }
    // check Y
    if (point.y() < minY || point.y() > maxY) {
      std::ostringstream oss;
      oss << "Y out of range: y=" << point.y()
          << " (valid [" << minY << "," << maxY << "]), idxY=" << idxY;
      throw std::out_of_range(oss.str());
    }
    // check Z
    if (point.z() < minZ || point.z() > maxZ) {
      std::ostringstream oss;
      oss << "Z out of range: z=" << point.z()
          << " (valid [" << minZ << "," << maxZ << "]), idxZ=" << idxZ;
      throw std::out_of_range(oss.str());
    }
